...
public class ParticleMultiway {
  ...
  public ParticleMultiway(double rx, double ry, double vx, double vy,
  ...
  public ParticleMultiway() {
    radius = 0.0015;
    final double MARGIN = .4;
    final double V_MAX = radius * 1.;
    final double theta = Math.random() * Math.PI * 2;
    final double v = V_MAX * Math.random();
    // Corrected initial position so particles aren't created less than 
    // radius distance from walls.
    rx = (1.0 - 2.0 * (radius + MARGIN)) * Math.random() + radius + MARGIN;
    ry = (1.0 - 2.0 * (radius + MARGIN)) * Math.random() + radius + MARGIN;
    vx = v * Math.cos(theta);
    vy = v * Math.sin(theta);
    mass = 0.5;
    color = Color.BLACK;
  ...
  public double timeToHit(ParticleMultiway b) {
    ParticleMultiway a = this;
  ...
  public void bounceOff(ParticleMultiway that) {
    ...
    double F = 2 * this.mass * that.mass * dvdr / ((this.mass + that.mass) *
                                                   dist);
    double fx = F * dx / dist;
    double fy = F * dy / dist;
